...
datafiletbl %<>% mutate(
	...
	is_insight = 
		(stat %>% str_detect('INSIGHT')) &
		(!( file %>% basename %>% str_detect('_sample_')))
)
...
datafiletbl  %>% write_tsv(config$results$input_metadata)




# -------------------------------------------------------------------------------
# --------Now plot the phylop and phascons data
# -------------------------------------------------------------------------------
...
insightfiles %>% map( . %>% fread %>% colnames) %>% unique
#rad in data and select the source/stat columns
measurelist <- c('E.A.','E.W.','alpha','rho')
measureerrlist <- measurelist %>% paste0(.,'_stderr')
loadinsight  <- function(x) x %>% fread %>% select_(.dots = c(list('dataID','thres'),measurelist,measureerrlist) )
...
#reshape the data
insight_data=map2(measurelist,measureerrlist,insight_data,.f = function(measurecol,errcol,insight_data){
	insight_data %>% 
		select(y = measurecol, stderr = errcol) %>% 
		mutate(ymin = y-stderr,ymax = y+stderr) %>% 
		mutate('measure'=!!measurecol) %>% 
		select(measure,y,ymin,ymax)
}) %>% bind_rows


#checkw e have unique ids
stopifnot(insight_data %>% group_by(data_source,dataID,thres,measure) %>% tally  %>% .$n %>% eq(1) %>% all)
...
# 
# source("/g/furlong/project/28_B_DNASE/analysis/evolutionary_analyses/INSIGHT/src/dermot/pleoitropy_evo_plots.R")