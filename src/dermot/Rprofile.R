...
#set the location of our libraries
projectlib <- "/g/furlong/Harnett/R"

.libPaths(projectlib)
options(import.path=c(projectlib))
cranpackages <- utils::available.packages(repo = "http://cran.rstudio.com/")
source("http://bioconductor.org/biocLite.R")

#this pretty much does what it says
install_or_load <- function(libname,cranurl="http://cran.rstudio.com/"){
  #try to load the package
  tryload = try(silent=TRUE, {library(warn.conflicts=FALSE,character.only=TRUE,libname)} )
  #if this fails,  
  if(methods::is(tryload,'try-error')){
    #biocLite it - this will install both bioConductor and cran packages
    biocLite(libname,lib.loc=projectlib,lib=projectlib,dependencies=TRUE)
    library(warn.conflicts=FALSE,character.only=TRUE,libname)
  }
  else{
